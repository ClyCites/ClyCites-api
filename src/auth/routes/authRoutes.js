...
import { protect, authorize, ownerOrAdmin } from '../middlewares/authMiddleware.js';
...
router.put('/users/:userId/role', authorize('admin'), asyncHandler(async (req, res, next) => {
  const { role } = req.body;
...
router.put('/users/:userId/status', authorize('admin'), asyncHandler(async (req, res, next) => {
